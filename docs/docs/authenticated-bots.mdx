...
Authenticated mode is a **deployment default with a per-request override**. Point `meeting-api` at
the provisioned session and set the default below; every stock `POST /bots` then spawns signed-in
with no hand-crafted bot config. A request carrying `authenticated: true` or `authenticated: false`
overrides the default for that spawn — which is what lets one deployment serve both signed-in and
guest bots (prefer the account, fall back to a guest when the session is busy or unconfigured).
...
With authenticated mode in effect for a spawn, the bot restores the stored session into its own
ephemeral browser profile before launch and joins as the signed-in account — no guest-name entry, no
lobby knock where the account has access. If a spawn asks for authenticated mode but the storage
config is incomplete, `POST /bots` refuses with a 503 naming what's missing — a half-configured
deployment never silently spawns anonymous bots. If the session store is unreachable at spawn time, the bot fails loud with a typed
